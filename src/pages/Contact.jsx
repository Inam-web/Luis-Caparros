  ...
  return (
    <>
      ...
      <div
        ...
        aria-atomic="true"
        className={cn(
          "fixed inset-0 z-[10000] flex items-center justify-center px-5",
          "transition-all duration-500",
          showSuccess ? "visible opacity-100" : "invisible opacity-0 pointer-events-none"
        )}
      >
        <div
          className={cn(
        ...
        <div
          className={cn(
            ...
            showSuccess ? "translate-y-0 scale-100" : "translate-y-8 scale-90"
          )}
        >
          <div className="relative w-32 h-32 flex items-center justify-center">
            ...
            />
            <div className={cn("absolute inset-0", showSuccess && "success-rays")}>
            ...
            >
              <div className="w-[62px] h-[62px] rounded-full border-2 border-white/30 flex items-center justify-center">
                <CheckIcon
                  className={cn(
                    "w-8 h-8 text-white",
          ...
          <div>
            ...
            >
              ...
              <div className="mt-8 grid sm:grid-cols-2 gap-x-6 gap-y-7">
                <div>
                  <label htmlFor="c-name" className="block text-[11px] font-bold tracking-[0.2em] uppercase text-pine-700 mb-1">
                    {t("contact.name")} *
                  </label>
                  <input
                    id="c-name"
                  ...
                  />
                  {errors.name && (
                    <p className="mt-1.5 text-xs font-semibold text-wine-600">{errors.name}</p>
                  )}
                </div>
                <div>
                  <label htmlFor="c-email" className="block text-[11px] font-bold tracking-[0.2em] uppercase text-pine-700 mb-1">
                    {t("contact.email")} *
                  </label>
                  <input
                    id="c-email"
                  ...
                  />
                  {errors.email && (
                    <p className="mt-1.5 text-xs font-semibold text-wine-600">{errors.email}</p>
                  )}
                </div>
                <div className="sm:col-span-2">
                  <label htmlFor="c-subject" className="block text-[11px] font-bold tracking-[0.2em] uppercase text-pine-700 mb-1">
                    {t("contact.subject")}
                  </label>
                  <select
                    id="c-subject"
                ...
                <div className="sm:col-span-2">
                  <label htmlFor="c-message" className="block text-[11px] font-bold tracking-[0.2em] uppercase text-pine-700 mb-1">
                    {t("contact.message")} *
                  </label>
                  <textarea
                    id="c-message"
                  ...
                  />
                  {errors.message && (
                    <p className="mt-1.5 text-xs font-semibold text-wine-600">{errors.message}</p>
                  )}
                </div>
                <div className="sm:col-span-2">
                  <label className="flex items-start gap-3 cursor-pointer select-none">
                    ...
                    <span className="text-sm text-pine-700">
                      {t("contact.privacy1")}
                      <Link to="/privacy-policy" className="link-ink font-semibold text-wine-600">
                  ...
                  {errors.privacy && (
                    <p className="mt-1.5 text-xs font-semibold text-wine-600">{errors.privacy}</p>
                  )}
                </div>
              </div>
              <button
            ...
            <div className="rv bg-pine-950 text-paper-50 p-8 relative overflow-hidden">
              ...
              <ul className="relative mt-5 space-y-4 text-sm">
                <li className="flex items-start gap-3">
                  ...
                  <a href={`mailto:${AUTHOR.email}`} className="hover:text-gold-300 transition-colors break-all">
                    {AUTHOR.email}
                  </a>
                </li>
                <li className="flex items-start gap-3">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0 pr-5">
              <h3 className="font-display font-semibold text-base text-pine-900">
                {lang === "en" ? "Message not sent" : "Mensaje no enviado"}
              </h3>
            ...
            <button
              type="button"
              onClick={closeToast}
              aria-label={lang === "en" ? "Close notification" : "Cerrar notificación"}
